...
type PanelTab = "PROBLEMS" | "OUTPUT" | "TERMINAL" | "DEBUG" | "PORTS";
...
const setExplorerDragPayload = (
  ...
  const encoded = `${EXPLORER_DND_TEXT_PREFIX}${encodeExplorerDragPayload(payload)}`;
...
const getExplorerDragPayload = (
  ...
  if (!raw) return null;
  const normalized = raw.startsWith(EXPLORER_DND_TEXT_PREFIX)
    ? raw.slice(EXPLORER_DND_TEXT_PREFIX.length)
    : raw;
  try {
    const parsed = JSON.parse(normalized) as ExplorerDragPayload;
    if (parsed?.entryPath && parsed?.entryType) return parsed;
  } catch {
...
const normalizeAiRanges = (ranges: AiRange[], content: string) => {
  const lines = content.split("\n");
  const lastLine = Math.max(1, lines.length);

  return ranges
    .map((range, idx) => {
      const safeId = range.id || `range-${idx}`;
      const startLineNumber = Math.min(
        lastLine,
        Math.max(1, Math.floor(range.startLineNumber)),
      );
      const startColumn = Math.max(1, Math.floor(range.startColumn));
      const endLineNumber = Math.min(
        lastLine,
        Math.max(startLineNumber, Math.floor(range.endLineNumber)),
      );
      const lineLength = lines[endLineNumber - 1]?.length ?? 0;
      const endColumn = Math.max(
        1,
        Math.min(lineLength + 1, Math.floor(range.endColumn)),
      );

      return {
        ...range,
        id: safeId,
        originalText: range.originalText ?? content,
        startLineNumber,
        startColumn,
        endLineNumber,
        endColumn,
...
const getFullRangeForCode = (code: string): AiRange => {
  ...
  const endColumn = Math.max(1, (lines[endLineNumber - 1]?.length ?? 0) + 1);
  return {
    id: "full",
    originalText: code,
    startLineNumber: 1,
...
const getRuntimeConfigForFilePath = (filePath: string | null) => {
  ...
  if (lower.endsWith(".rs"))
    return { image: "rust:latest", command: "cargo run" };
  ...
  if (lower.endsWith(".ts") || lower.endsWith(".tsx"))
    return { image: "node:20-alpine", command: "npm run dev" };
  if (lower.endsWith(".js"))
    return { image: "node:20-alpine", command: "node index.js" };
  if (lower.endsWith(".go"))
    return { image: "golang:1.22-alpine", command: "go run ." };
  return { image: "node:20-alpine", command: "" };
};
            ...
            <li key={node.path}>
              ...
              >
                <button
                  ...
                  draggable
                  onDragStart={(event) =>
                    handleDragStart(event, node.path, "directory")
                  }
                  onDragOver={(event) => {
                  ...
                  onContextMenu={(event) =>
                    onOpenContextMenu(event, node.path, "directory")
                  }
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  <span
                    style={{ fontSize: 10, color: "#6e7681", minWidth: 12 }}
                  >
                    {isOpen ? "▾" : "▸"}
                  </span>
                  <span
        ...
        return (
          <li key={node.path}>
            <button
              ...
              onDrop={(event) =>
                handleDrop(event, getParentFolderPath(node.path))
              }
              data-repo-drop-target="file"
              data-repo-path={node.path}
              onClick={() => onSelectFile(node.path)}
              onContextMenu={(event) =>
                onOpenContextMenu(event, node.path, "file")
              }
              style={{
                display: "flex",
                alignItems: "center",
    ...
    async (entryPath: string, entryType: "file" | "directory") => {
      ...
      try {
        ...
        const selectedIsRenamed =
          ...
          (currentSelectedPath === entryPath ||
            currentSelectedPath.startsWith(`${entryPath}/`));

        if (selectedIsRenamed && currentSelectedPath && data.destinationPath) {
          const nextSelected = remapPathAfterMove(
            currentSelectedPath,
            entryPath,
            data.destinationPath,
          );
      ...
      try {
        ...
        if (selectedIsMoved && currentSelectedPath) {
          ...
          if (reloadResponse.ok) {
            ...
            const canonicalRanges = normalizeAiRanges(
              reloadData.aiRanges ?? [],
              canonicalContent,
            );
            lastPersistedSnapshotRef.current = {
    ...
    (e: DragEvent<HTMLElement>) => {
      ...
      if (internalPayload) {
        ...
        handleDropEntryToFolder(
          internalPayload.entryPath,
          internalPayload.entryType,
    ...
    const handleWindowDragEnd = () => {
      ...
      if (!dragMoveTriggeredRef.current) {
        ...
        if (destinationFolderPath !== sourceParent) {
          handleDropEntryToFolder(
            activePayload.entryPath,
            activePayload.entryType,
            destinationFolderPath,
          );
        }
      }
      ...
      >
        <div style={{ flex: 1, display: "grid", placeItems: "center" }}>
          <div style={{ textAlign: "center" }}>
            <p style={{ color: "#f85149", fontSize: 14, marginBottom: 16 }}>
              {errorMessage}
            </p>
            <Link href="/workspace" style={{ color: "#58a6ff", fontSize: 13 }}>
              ← Back to repos
            </Link>
          </div>
        </div>
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ color: "#f0b72f", fontSize: 14 }}>●</span>
        ...
        <div style={{ flex: 1 }} />
        {fileMessage && (
          <span style={{ fontSize: 11, color: "#3fb950" }}>{fileMessage}</span>
        )}
        <button
          ...
          style={{
            ...
            color: isSavingFile ? "#8b949e" : "#e6edf3",
            fontSize: 12,
            cursor: "pointer",
          }}
        >
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 6,
            border: "1px solid #30363d",
            background: chatOpen ? "rgba(88,166,255,0.12)" : "transparent",
            color: "#58a6ff",
            fontSize: 12,
            cursor: "pointer",
          }}
          ...
          <div style={{ position: "relative" }}>
            ...
            {inviteOpen && (
              ...
              >
                ...
                {inviteError && (
                  <p style={{ fontSize: 11, color: "#f85149", marginTop: 6 }}>
                    {inviteError}
                  </p>
                )}
                {inviteMessage && (
                  <p style={{ fontSize: 11, color: "#3fb950", marginTop: 6 }}>
                    {inviteMessage}
                  </p>
                )}
                {invites.length > 0 && (
          ...
          >
            ...
            >
              {isLoadingFiles ? (
                ...
                >
                  No files yet. Drop files here or click +
                </p>
              ) : (
                <FileTree
                  nodes={fileTree}
          ...
          >
            ...
            >
              ...
              >
                {selectedFilePath ? (
                  ...
                  >
                    <FileIcon name={getBaseName(selectedFilePath)} />
                    <span
                      style={{
                        fontSize: 13,
                        color: "#e6edf3",
                    ...
                    <span
                      style={{ fontSize: 11, color: "#8b949e", marginLeft: 2 }}
                    >
                      {editorLanguage}
                    </span>
                  </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: terminalOpen ? "#e6edf3" : "#8b949e",
                    fontSize: 12,
                    cursor: "pointer",
                    whiteSpace: "nowrap",
                  }}
                >
                  {terminalOpen ? "Panel ▾" : "Panel ▸"}
                </button>
              </div>
              ...
              >
                ...
                <div style={{ flex: 1 }} />
                <span style={{ color: "#8b949e", fontSize: 11 }}>
                  History: {selectedVersionTimeLabel}
                </span>
                <span style={{ color: "#8b949e", fontSize: 11 }}>
                  Lang: {editorLanguage}
                </span>
                ...
                >
                  {selectedFilePath ? (
                    <Editor
                      ...
                      onAiRangesChange={(ranges) =>
                        handleEditorAiRangesChange(selectedFilePath, ranges)
                      }
                      embedded
                    />
                  ) : (
                    ...
                    >
                      <div style={{ textAlign: "center" }}>
                        ...
                        <p style={{ color: "#6e7681", fontSize: 12 }}>
                          Pick a file from the explorer or drop files anywhere
                        </p>
                      </div>
                    </div>
                  )}
                      ...
                      {(
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background:
                              panelTab === tab ? "#161b22" : "transparent",
                    ...
                    <div style={{ flex: 1, minHeight: 0, overflow: "hidden" }}>
                      {panelTab === "TERMINAL" ? (
                        ...
                        >
                          ...
                          {panelTab === "OUTPUT" && "Output is empty."}
                          {panelTab === "DEBUG" && "Debug console ready."}
              ...
              >
                ...
                >
                  ...
                  <span style={{ flexShrink: 0 }}>Lines: {lineCount}</span>
                  <span style={{ flexShrink: 0 }}>Lang: {editorLanguage}</span>
                  <span style={{ flexShrink: 0 }}>
                    Image: {runtimeDefaults.image}
                  </span>
                </div>
                <span
                  title={collaborationRoomId}
                  style={{
                    flexShrink: 0,
                    maxWidth: "35%",
                    overflow: "hidden",
            ...
            <div
              style={{
                ...
                borderLeft: chatOpen ? "1px solid #21262d" : "none",
                background: "#0f141b",
                overflow: "hidden",
                display: "flex",
                flexDirection: "column",
