...
const normalizeAiRanges = (
  ...
  code = "",
): AiRange[] => ranges.map((range) => normalizeAiRange(range, code));
...
export default function RepoEditorPage() {
  ...
  const handleSaveFile = async () => {
    if (!ownerUid || !selectedFilePath) return;
    if (loadedSelectedFilePath !== selectedFilePath || isLoadingSelectedFile)
      return;
    ...
    try {
      ...
      if (
        ...
        const confirmed = window.confirm(
          "This file is currently empty. Save will overwrite existing content. Continue?",
        );
        if (!confirmed) {
          return;
        }
          ...
          <div style={{ position: "relative" }}>
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border: "1px solid #30363d",
                background: inviteOpen ? "rgba(88,166,255,0.1)" : "transparent",
                color: "#58a6ff",
                fontSize: 12,
                cursor: "pointer",
              }}
      ...
      {selectedFilePath && fileVersions.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: 11, color: "#8b949e" }}>
              Selected: {selectedVersionTimeLabel}
            </span>
            <div style={{ flex: 1 }} />
            <button
              type="button"
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border: "1px solid #30363d",
                background: isRestoringVersion ? "#30363d" : "#1f6feb",
                color: "#fff",
                fontSize: 11,
                cursor: "pointer",
                opacity:
                  isRestoringVersion || selectedVersionIndex == null ? 0.7 : 1,
              }}
            >
          ...
          {isLoadingVersionPreview ? (
            <p style={{ marginTop: 6, fontSize: 11, color: "#8b949e" }}>
              Loading preview…
            </p>
          ) : previewVersionContent ? (
            <pre
            ...
            <p style={{ marginTop: 6, fontSize: 11, color: "#6e7681" }}>
              Move the slider to preview an older version, then click “Restore
              this version”.
            </p>
          )}
        </div>
      ...
      >
        ...
        >
          ...
          >
            {isLoadingFiles ? (
              <p
                style={{ fontSize: 12, color: "#8b949e", padding: "8px 12px" }}
              >
                Loading…
              </p>
            ) : fileTree.length === 0 ? (
          ...
          >
            {selectedFilePath ? (
              ...
              >
                ...
                <span style={{ fontSize: 11, color: "#8b949e", marginLeft: 2 }}>
                  {editorLanguage}
                </span>
              </div>
            ) : (
              <span
                style={{ fontSize: 12, color: "#8b949e", padding: "0 14px" }}
              >
                No file open
              </span>
            )}
          ...
          >
            <div
              style={{
                ...
                minWidth: 0,
                borderRight: isHtmlPreviewFile ? "1px solid #21262d" : "none",
              }}
            >
              {selectedFilePath ? (
                isLoadingSelectedFile ||
