...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"account" | "repos" | "pet">(
    "account",
  );
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        setParticipatingRepos([]);
        setMetaMessage("Could not load repos");
      }
    });
    return unsub;
  }, []);
  ...
  const ownerCount = useMemo(
    () => participatingRepos.filter((r) => r.role === "owner").length,
    [participatingRepos],
  );
  const collabCount = useMemo(
    () => participatingRepos.filter((r) => r.role === "collaborator").length,
    [participatingRepos],
  );
  ...
  const visibleRepos = useMemo(() => {
    ...
    return participatingRepos
      .filter((r) => {
        if (
          repoRoleFilter !== "all" &&
          (r.role ?? "collaborator") !== repoRoleFilter
        )
          return false;
        if (!q) return true;
      ...
      .sort((a, b) =>
        repoSort === "owner"
          ? getOwnerUsername(a).localeCompare(getOwnerUsername(b))
  ...
  const handleCopyRepoId = async (repoId: string) => {
    try {
      ...
      setErrorMessage("Could not copy");
    }
  };
  ...
  const handlePetEnabledChange = (v: boolean) => {
    setPetEnabled(v);
    window.localStorage.setItem(PET_ENABLED_KEY, String(v));
    emitPetSettingsChanged();
  };
  const handlePetSkinChange = (v: PetSkin) => {
    setPetSkin(v);
    window.localStorage.setItem(PET_SKIN_KEY, v);
    emitPetSettingsChanged();
  };
  const handlePetMoodChange = (v: PetMood) => {
    setPetMood(v);
    window.localStorage.setItem(PET_MOOD_KEY, v);
    emitPetSettingsChanged();
  };
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
              ...
              <StatPill
                label="total"
                value={participatingRepos.length}
                color="#3fb950"
              />
            </div>
          </div>
            ...
            {activeTab === "account" && (
              ...
              >
                <GlassRow
                  label="User ID"
                  value={user?.uid ?? "–"}
                  onCopy={() => void handleCopyText(user?.uid ?? "", "uid")}
                  copied={copiedText === "uid"}
                />
                <GlassRow
                  label="Email"
                  value={user?.email ?? "–"}
                  onCopy={() => void handleCopyText(user?.email ?? "", "email")}
                  copied={copiedText === "email"}
                />
                <GlassRow
                  label="Role mix"
                  value={`${ownerCount} owner · ${collabCount} collab`}
                />
              ...
              >
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <GlassSelect
                    ...
                    onChange={(v) =>
                      setRepoRoleFilter(v as typeof repoRoleFilter)
                    }
                    options={[
                      ["all", "All"],
                      ["owner", "Owner"],
                  ...
                  <GlassSelect
                    value={repoSort}
                    onChange={(v) => setRepoSort(v as typeof repoSort)}
                    options={[
                      ["name", "By name"],
                      ["owner", "By owner"],
                    ]}
                ...
                {visibleRepos.map((repo) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        background:
                          copiedRepoId === repo.id ? "#1f6feb" : "transparent",
          ...
          >
            ...
            >
              ...
              <span
                style={{
                  color: "transparent",
                  WebkitTextStroke: "1px rgba(88,166,255,.5)",
                  fontSize: "0.65em",
                }}
              >
                {user?.email ?? ""}
...
function StatPill({
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 9, color: "#8b949e", letterSpacing: "0.08em" }}>
        {label}
      </span>
    </div>
  );
    ...
    >
      ...
      {onCopy && (
        <button
          onClick={onCopy}
          style={{
            background: copied ? "#1f6feb" : "transparent",
            border: "1px solid #30363d",
