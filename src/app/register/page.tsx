...
export default function RegisterPage() {
  ...
  // ── canvas — green-tinted for register (vs blue on landing)
  useEffect(() => {
    ...
    let W = 0, H = 0, mx = 0, my = 0, raf: number;
    ...
    const draw = () => {
      ...
      // green glow — differentiated from landing's blue
      const g1 = ctx.createRadialGradient(W * 0.8, H * 0.2, 0, W * 0.8, H * 0.2, W * 0.35);
      g1.addColorStop(0, "rgba(63,185,80,.06)"); g1.addColorStop(1, "transparent");
      ctx.fillStyle = g1; ctx.fillRect(0, 0, W, H);

      const g2 = ctx.createRadialGradient(W * 0.2, H * 0.75, 0, W * 0.2, H * 0.75, W * 0.28);
      g2.addColorStop(0, "rgba(88,166,255,.05)"); g2.addColorStop(1, "transparent");
      ...
      particles.forEach(p => {
        ...
        p.vx *= 0.82; p.vy *= 0.82;
        p.x += p.vx; p.y += p.vy;
        // green dots instead of blue
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.s, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(63,185,80,${0.10 + force * 0.25})`;
        ctx.fill();
      });
  ...
  const strengthColor = ["", "#f85149", "#e3b341", "#58a6ff", "#3fb950"];

  const handleRegister = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!auth) { setErrorMessage("Firebase is not configured yet."); return; }
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : "Unable to continue with Google.");
      setIsSubmitting(false);
    }
    setIsSubmitting(false);
  };
  ...
  if (firebaseConfigError) {
    return (
      <main style={{ flex: 1, padding: 32, maxWidth: 640, margin: "0 auto" }}>
        <h1 style={{ fontSize: 22, fontWeight: 600, marginBottom: 16 }}>Firebase Setup Required</h1>
  ...
  const inputStyle = (field: string): React.CSSProperties => ({
    ...
    boxShadow: focusedField === field ? "0 0 0 3px rgba(63,185,80,.12)" : "none",
    cursor: "none",
  });
  ...
  return (
    <main style={{
      ...
      <div ref={cursorRef} style={{
        ...
        background: "#3fb950", borderRadius: "50%",
      ...
      <div style={{
        ...
        <div style={{
          ...
          marginBottom: 24, animation: "fadein .5s .3s both",
        }}>
          <span style={{
            display: "block", width: 20, height: 1, background: "#3fb950",
          }} />
          <span style={{ fontSize: 10, color: "#3fb950", letterSpacing: "0.3em", fontWeight: 700 }}>
            NEW ACCOUNT
          </span>
          <span style={{
            marginLeft: "auto", fontSize: 10, color: "#484f58", letterSpacing: "0.1em",
          }}>
            iTECify · v0.1.0
          </span>
        </div>
        ...
        <div style={{
          ...
          <span style={{ marginLeft: "auto", marginRight: "auto", fontSize: 11, color: "#8b949e" }}>
            auth · register · new session
          </span>
        </div>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 24 }}>
            <div style={{ fontSize: 10, color: "#8b949e", marginBottom: 6, fontFamily: "'JetBrains Mono',monospace" }}>
              <span style={{ color: "#3fb950" }}>✦</span> itecify init --user
            </div>
            <h1 style={{
              fontFamily: "'Syne', sans-serif",
              fontSize: 26, fontWeight: 800,
              letterSpacing: "-0.03em", color: "#e6edf3",
              lineHeight: 1.1,
            }}>
              Create your<br />
          ...
          <form onSubmit={handleRegister}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12, cursor: "none", padding: 0,
                ...
                <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} style={{
                      ...
                      background: i <= strength ? strengthColor[strength] : "#21262d",
                      transition: "background .3s",
                    }} />
                  ))}
                </div>
                <div style={{ fontSize: 10, color: strengthColor[strength] }}>
                  {strengthLabel[strength]}
                </div>
              </div>
            )}
            {password.length === 0 && <div style={{ marginBottom: 20 }} />}
            ...
            <div style={{
              ...
              margin: "14px 0", color: "#30363d", fontSize: 11,
            }}>
        ...
        <div style={{
          ...
          By continuing you agree to iTECify's Terms of Service
        </div>
      </div>
