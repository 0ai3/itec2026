...
export default function Editor({
	...
	const handleDragStart = useCallback((e: React.MouseEvent, rangeId: string) => {
		...
		const onMouseMove = (ev: MouseEvent) => {
			...
			// Remove block from old position, insert at new
			const oldFullRange = {
				startLineNumber: currentRange.startLineNumber,
				startColumn: 1,
				endLineNumber: currentRange.endLineNumber + (currentRange.startLineNumber > 1 ? 0 : 1),
				endColumn: currentRange.startLineNumber > 1
					? model.getLineMaxColumn(currentRange.endLineNumber)
					: model.getLineMaxColumn(currentRange.endLineNumber + 1),
			}

			// Simpler approach: swap lines
			...
			// Do actual edit: move lines in model
	...
	}, [transportRoomId, language, addAiRange, applyAiRanges, fixSelectedCode])
	...
	const selectionPopupJsx = selectionPopup && (
		<div
			...
			// Only preventDefault on the wrapper divs, NOT on the input
			onMouseDown={(e) => {
				// Allow input and buttons to receive native focus
